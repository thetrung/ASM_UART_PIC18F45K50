...
# 1 "/Users/thetrung/.mchp_packs/Microchip/PIC18F-K_DFP/1.15.303/xc8/pic/include/pic18.inc" 1 3





# 1 "/Users/thetrung/.mchp_packs/Microchip/PIC18F-K_DFP/1.15.303/xc8/pic/include/pic18_chip_select.inc" 1 3
# 214 "/Users/thetrung/.mchp_packs/Microchip/PIC18F-K_DFP/1.15.303/xc8/pic/include/pic18_chip_select.inc" 3
# 1 "/Users/thetrung/.mchp_packs/Microchip/PIC18F-K_DFP/1.15.303/xc8/pic/include/proc/pic18f45k50.inc" 1 3
# 47 "/Users/thetrung/.mchp_packs/Microchip/PIC18F-K_DFP/1.15.303/xc8/pic/include/proc/pic18f45k50.inc" 3
SRCON1 equ 0F57h
...
TOSU equ 0FFFh
# 8585 "/Users/thetrung/.mchp_packs/Microchip/PIC18F-K_DFP/1.15.303/xc8/pic/include/proc/pic18f45k50.inc" 3
...
# 215 "/Users/thetrung/.mchp_packs/Microchip/PIC18F-K_DFP/1.15.303/xc8/pic/include/pic18_chip_select.inc" 2 3
# 7 "/Users/thetrung/.mchp_packs/Microchip/PIC18F-K_DFP/1.15.303/xc8/pic/include/pic18.inc" 2 3
...
uart_init:
    ...
    bsf BAUDCON1, 6; ((BAUDCON1) and 0FFh), 6, a = 1
                   ; Mean: don't IDLE when not receiving anything.

    ; Enable UART by ((RCSTA1) and 0FFh), 7, a :
    ; RCSTA1 = 0x80 = 1000 0000
    bsf RCSTA1, 7 ; ((RCSTA1) and 0FFh), 7, a = 1

    ; TXSTA1 = 0x26 = 0010 0110
    bsf TXSTA1, 5 ; ((TXSTA1) and 0FFh), 5, a : ((PORTC) and 0FFh), 6, a is ready(1).
    bsf TXSTA1, 2 ; ((TXSTA1) and 0FFh), 2, a : ((PORTC) and 0FFh), 6, a is ready(2).
    bsf TXSTA1, 1 ; ((TXSTA1) and 0FFh), 1, a : ((PORTC) and 0FFh), 6, a is done.
...
uart_enable:
    ; To enable serial com (((PORTC) and 0FFh), 7, a/((PORTC) and 0FFh), 6, a) :
    bsf RCSTA1, 7 ; ((RCSTA1) and 0FFh), 7, a = 1
    return

uart_disable:
    ; To disable serial com (((PORTC) and 0FFh), 7, a/((PORTC) and 0FFh), 6, a) :
    bcf RCSTA1, 7 ; ((RCSTA1) and 0FFh), 7, a = 0
    return

uart_write:
uart_tx_ready: ; when PIR1.((PIR1) and 0FFh), 4, a && TXTSA1.((TXSTA1) and 0FFh), 5, a
    btfss PIR1, 4 ; PIR1.((PIR1) and 0FFh), 4, a == 1 ?
    goto uart_tx_ready
    btfss TXSTA1, 5 ; TXTSA1.((TXSTA1) and 0FFh), 5, a == 1 ?
    goto uart_tx_ready

uart_send_byte:
    movwf TXREG1 ; TXREG1 = a byte to write
uart_is_tx_done:
    btfss TXSTA1, 1 ; ((TXSTA1) and 0FFh), 1, a = 1 ? Tx is done.
...
;=======================================
;; PROGMEM init1 ::
    oled_write 0xAE ; 1. Display off
    oled_write 0xD5 ; 2. Set Clock divide ratio (0x00=1) and oscillator frequency (0x8)
    oled_write 0x80 ; suggested ratio
    oled_write 0xA8 ; 3. multiplex ratio
;; PROGMEM init2 ::
    oled_write 0xD3 ; 4. display offset
    oled_write 0x00
    oled_write 0x40 | 0x0
    oled_write 0x8D ; 6. charge pump setting (p62): 0x014 enable, 0x010 disable, SSD1306 only, should be removed for SH1106 */
;; VCC MODE ::
    oled_write 0x02 ; 0x01 for EXT & 0x02 for SWC
;; PROGMENT init3 ::
    oled_write 0x20 ; 0x20 - Memory Address Mode: horizontal
...
; 128x64
; comPins = 0x12;
; contrast = 0x01 ? 0x9F : 0xCF;
; 96x16
; comPins = 0x2;
; contrast = 0x01 ? 0x10 : 0xAF;
    oled_write 0xDA
...
;; PROGMEM INIT5 ::
    oled_write 0xDB ; 0xDB
    oled_write 0x40 ;
    oled_write 0xA4 ; 0xA4
    oled_write 0xA6 ; 0xA6
    oled_write 0x2E
    oled_write 0xAF
;; ((SSP1STAT) and 0FFh), 4, a I2C ;;
    call i2c_stop
    return
...
delay_1ms:
    ...
    mov T1CON, 0x01
wait_TMR1IF:
    btfss PIR1, 0 ; ((PIR1) and 0FFh), 0, a
    goto wait_TMR1IF
